...
class BabelTranslator(Translator):
    def __init__(self):
        ...
        for locale in settings.TRANSLATIONS:
            self.translations[locale] = Translations.load(
                settings.BASE_DIR / "locales", [locale]
            )
    ...
    def get_translation(self, string: str, locale: str) -> str | None:
        """ "Synchronous method to get translations outside of discord.py's async
        translation system."""
...
def set_locale(obj: Interaction | str):
    """Helper function to set the current locale to either a locale string or to extract
    it from an Interaction object."""
...
def localize_fmt(string: str, /, **kwargs) -> str:
    """Helper function to translate and format bare strings according to current context
    locale."""
